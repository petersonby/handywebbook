...
Ничего своего: повторение mdn, спецификации, документации и статьи, которую вы когда-то читали.
...
Существуют инструменты, которые позволяют автоматизировать этот процесс, например [Critical](https://github.com/addyosmani/critical) от Эдди Османи.
...
Все современные браузеры, кроме IE9-, поддерживают __CSS transitions__ и __CSS animations__, которые позволяют реализовать анимацию средствами CSS, без JavaScript.

#### CSS Transition

__CSS переходы__ позволяют анимировать переход css-свойств из исходного состояния в конечное. Делается это при помощи свойства `transition`. 

Данное свойство является сокращённой записью нескольких свойств:
...
__CSS анимации__ создаются при помощи свойства `animation`, а также правила `@keyframes`. Как и в случае `transition`, `animation` является кратким свойством, описывающим анимацию. 
...
Существует много способов вставки svg. Их можно разделить на два типа:
...
Внутри такого спрайта svg-файлы находятся в теге `<symbol>` и имеют уникальный `id`. А на странице подключаются с помощью тега `<use xlink:href="#id">`
...
По состоянию на 2020 год, на мобильные телефоны приходится 53,3% всех запросов в интернете. 

Использование __Mobile First__ подхода даёт следующие преимущества:
...
- Упрощение добавления адаптивности
- Уменьшение вероятности "поломки" сайта на каком-то разрешении экрана
...
Задача создания доступных интерфейсов относится __не только__ к разработчику, но также __как минимум__ к дизайнеру и менеджеру. 

В настоящее время создаются и утверждаются законы и гайдлайны по веб-доступности на уровне различных стран, организаций и крупных компаний. В некоторых странах, например в странах ЕС, существует административная ответственность в случае нарушения законов о доступности.
