...
Ответы на вопросы с собеседований по HTML, CSS и JS.  
Простые ответы на разные вопросы.  
Повторение mdn, спецификаций, и статьи, что вы когда-то читали, ничего нового.
...
- [RWD](#rwd)
- [A11y (Доступность)](#a11y-доступность)
...
A11y (A __ccessibilit__ y) – регламентирует лучшие практики обеспечения работоспособности и доступности сайта вне зависимости от физических и технических ограничений.

Следует знать, что задача создания доступных интерфейсов относится __не только__ к разработчику, но также __как минимум__ к дизайнеру и менеджеру. В настоящее время создаются и утверждаются законы и гайдлайны по веб-доступности на уровне различных стран, организаций и крупных компаний. В некоторых странах, например в странах ЕС, существует административная ответственность в случае нарушения законов о доступности.

Рассмотрим, что относится к веб-разработчикам при обеспечении доступности:
- Правильное использование семантики HTML5
- Расстановка `h1`-`h6` заголовков
- Адаптивность сайта или приложения
- Доступность с клавиатуры
- Логичное перемещение фокуса
- Использование атрибутов `alt` для картинок
- Использование `label`, атрибутов `title` или `aria-label` для полей ввода
- Использование `aria-` атрибутов и атрибутов `role`
- И другое

Для тестирования доступности интерфейсов могут использоваться различные помощники, как автоматизированные (как __LightHouse__), так и различные скринридеры (например __VoiceOver__) для ручного тестирования.

### Регрессионное тестирование

__Регрессионное тестирование__ (лат. regressio «обратное движение, возвращение») – способ тестирования, направленный на проверку того факта, что уже имеющаяся функциональность работает как раньше. 

Предположим у нас есть готовый код, который работает как нужно. Но, при добавлении или изменении фрагмента кода в одном месте, внезапно "слетает" другой фрагмент, что остаётся незамеченным. Здесь на помощь и приходит _регрессионное тестирование_. 

Применительно к _вёрстке_, вкратце создаются скрины "идеальной вёрстки", а затем скрины после изменений в коде. Далее эти скрины накладываются друг на друга, и выявляются несоответствия. Если это то, чего мы добивались, то скрины после изменений становятся эталонными, в противном случае необходимо править баги и повторять процедуру.

Существует несколько инструментов, например: PhantomCSS, BackstopJS, Selenium, Gemini и другие.
